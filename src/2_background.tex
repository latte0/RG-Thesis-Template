...
すなわち、$ \mathrm{E}[Y|X] = X \cdot W $ に対して$ G(\mathrm{E}[Y|X]) = X \cdot W $ となるような$ G $ を取り入流。
...
セミパラメトリックにモデルを推定する手法は統計学ではIchimura~\cite{ichimura}から始まり、PAVアルゴリズムとしてAdam Tauman Kalai(2008)~\cite{isotron}によりその有用性が確かめられた。
