...
関数の形式はカーネル密度推定であり、入力の出力を一つの式で表現することができる。
...
関数の形式はカーネル密度推定を用いることで、入力に対しての出力を一つの式で表せるようにする。そうすることでディープラーニングでも使えるぐらいの少ない計算コストが実現できる。
...
本研究の提案手法はIchimuraの手法と
わかりやすい図は\ref{proposed_method}に示した。
