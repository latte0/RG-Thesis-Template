...
1000回実験する中で発散頻度(エラーの回数)の回数を記録する実験をこなう。
そして、各ニューラルネットの構成で記録しどの組み合わせが一番エラー回数が少ないか調査する。
また、実験3.1ではLearningRateは小さい方がエラー確率が減ることは自明なためLRは$ 10^{-5} $で固定するものとする。




\subsection{実験3.2 クリッピングを用いた勾配発散の制御}
実験3.2では表(\label{exp:boston})の設定1の条件に対してクリッピングを用いる場合とそうでない場合にエラーの回数がどのように変化するか実験をする。
また学習性能についての評価を行うため、エラーが発生しなかった場合のAccuracyを平均値を用いた比較を行う。
